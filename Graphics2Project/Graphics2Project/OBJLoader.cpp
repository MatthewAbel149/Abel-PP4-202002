	...
	std::vector < DirectX::XMFLOAT3 > &out_normals)
{
	...
	std::vector<OBJ_VERTEX> vertexOutput;
	...
	if (true) {
		for (unsigned int i = 0; i < vertexIndices.size(); ++i)
		{
			...
			tempVertexOutput.pos = vertex[i];
			tempVertexOutput.nrm = normal[i];
			tempVertexOutput.tex = texels[i];

			vertexOutput.push_back(tempVertexOutput);
		}
	}
